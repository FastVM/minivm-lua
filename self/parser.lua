...
lua.parens = parser.select(2, lua.ignore(parser.exact('(')), lua.expr, lua.ignore(parser.exact(')')))
...
lua.pre = parser.first(lua.ast('length', lua.ignore(parser.exact('#')), lua.post), lua.ast('not', lua.keyword('not'), lua.pre), lua.ast('neg', lua.ignore(parser.exact('-')), lua.post), lua.post)
